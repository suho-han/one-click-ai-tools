...
- menubar popover에는 terminal-launch action을 직접 노출하지 않는다.
- menubar footer는 app-local action만 둔다.
  - primary: `Refresh now`
  - navigation: `Settings`
  - exit: `Quit`
- terminal을 여는 action은 `Settings` 안의 별도 `CLI Actions` 섹션으로 이동한다.
  - `Open Usage`
  - `Open Monitor`
  - `Run Session Refresh`
  - `Run Alert Check`
- `Settings`에는 appearance toggle도 둔다.
  - `Use provider accent colors`
