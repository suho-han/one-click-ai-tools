...
func TestRenderWaybarJSON(t *testing.T) {
	...
	if err := RenderWaybarJSON(&buf, results); err != nil {
...
func TestRenderPolybarLineEscapesPercentAndColorsSeverity(t *testing.T) {
	...
	if err := RenderPolybarLine(&buf, results); err != nil {
		t.Fatalf("RenderPolybarLine: %v", err)
	}
	out := buf.String()
	// Literal "%" must be doubled for polybar; the token whose raw value is
	// 90%-used carries the warn color while the ok token stays unstyled
	// (display values are remaining: 20 used -> 80 left, 90 used -> 10 left).
	if !strings.Contains(out, `X-80%%`) {
		t.Fatalf("output = %q, want escaped percent on ok token", out)
	}
	if !strings.Contains(out, "%{F"+severityColorWarn+"}C-10%%%{F-}") {
		t.Fatalf("output = %q, want warn-colored token", out)
	}
...
func TestRenderSwiftBarProtocol(t *testing.T) {
	var buf bytes.Buffer
	results := sampleStatuslineResults()
	if err := RenderSwiftBar(&buf, results); err != nil {
	...
	if !strings.HasPrefix(lines[0], "X-45% C-88% | color=") {
...
func TestStatuslineRenderersHandleEmptyResults(t *testing.T) {
	var buf bytes.Buffer
	if err := RenderWaybarJSON(&buf, nil); err != nil {
	...
	buf.Reset()
	if err := RenderPolybarLine(&buf, nil); err != nil {
	...
	buf.Reset()
	if err := RenderSwiftBar(&buf, nil); err != nil {
...
func TestStatuslineFiltersUnconfiguredProviders(t *testing.T) {
	...
	// codex's compact value comes from the 7d bucket (55 used -> 45 left);
	// copilot stays as the visible "?"-token error.
	if payload.Text != "X-45% P-?" {
	...
	buf.Reset()
	if err := RenderWaybarJSON(&buf, []UsageResult{usable, noAuth, emptyWindow}); err != nil {
		t.Fatalf("RenderWaybarJSON: %v", err)
	}
	payload = struct {
		Text    string `json:"text"`
	...
	if payload.Text != "X-45%" || payload.Class != "ok" {
